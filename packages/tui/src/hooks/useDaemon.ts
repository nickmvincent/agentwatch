...
export function useDaemon(daemonUrl: string, paused: boolean): DaemonState {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [agentsRes, reposRes, hooksRes, portsRes] = await Promise.all([
        fetch(`${daemonUrl}/api/agents`),
        fetch(`${daemonUrl}/api/repos`),
        fetch(`${daemonUrl}/api/hooks/sessions?limit=50`),
        fetch(`${daemonUrl}/api/ports`)
      ]);
