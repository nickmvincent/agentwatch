...
// Hooks Timeline view component with expandable entries
function HooksTimelineView({ timeline }: { timeline: ToolUsage[] }) {
    ...
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-1">
          {Object.entries(toolSummary)
            ...
            .map(([tool, count]) => (
              ...
              >
                {tool} <span className="text-gray-500">x{count}</span>
              </span>
            ))}
        </div>
      </div>
      ...
      <div className="space-y-2">
        {timeline.map((usage, idx) => {
          ...
          const hasDetails =
            usage.tool_input || usage.tool_response || usage.error;
          ...
          return (
            <div
              ...
              } ${hasDetails ? "cursor-pointer hover:bg-gray-600/30" : ""}`}
              ...
              <div className="p-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2 text-xs text-gray-500">
                    ...
                    <span>{formatTimeTs(usage.timestamp)}</span>
                  </div>
                </div>
                {usage.error && !isExpanded && (
                ...
                <div className="px-3 pb-3 space-y-2 border-t border-gray-600/50">
                  {usage.tool_input && (
                    <div className="mt-2">
                      ...
                      <pre className="p-2 bg-gray-900/50 rounded text-xs font-mono text-gray-300 overflow-x-auto max-h-40 overflow-y-auto">
                        {JSON.stringify(usage.tool_input, null, 2)}
                      </pre>
                    </div>
                  )}
                  {usage.tool_response && (
